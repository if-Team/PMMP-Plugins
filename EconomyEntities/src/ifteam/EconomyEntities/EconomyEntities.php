...
namespace ifteam\EconomyEntities;
...
class EconomyEntities extends PluginBase implements Listener {
	...
	public function initSpawn() {
		...
		for($i = 1; $i <= $botspawncount; $i ++) {
			if (! isset ( $this->config ["BotSpawnList"] [$i] )) return;
	...
	public function BotSpawn($x, $y, $z, $i) {
		...
		$nbt = new Compound ( "", [ "Pos" => new Enum ( "Pos", [ new Double ( "", $x ),new Double ( "", $y ),new Double ( "", $z ) ] ),"Motion" => new Enum ( "Motion", [ new Double ( "", 0 ),new Double ( "", 0 ),new Double ( "", 0 ) ] ),"Rotation" => new Enum ( "Rotation", [ new Float ( "", 0 ),new Float ( "", 0 ) ] ) ] );
	...
	public function BotUpdate() {
		for($i = 1; $i <= $this->config ["BotSpawnCount"]; $i ++) {
			...
			if ($this->botspawnlist [$i] ["Ent"]->dead) {
				...
				$this->getServer ()->getScheduler ()->scheduleDelayedTask ( new CallbackTask ( [ $this,"Respawn" ], [ $i ] ), 20 * $this->config ['RespawnTime'] );
				continue;
			}
			$pos = explode ( ":", $this->config ["BotSpawnList"] [$i] ["pos"] );
			$nearby = 0;
	...
	public function Villager_attack($i, $player, $f) {
		...
		if ($this->botspawnlist [$i] ["attackcool"] >= 6) {
			$nbt = new Compound ( "", [ "Pos" => new Enum ( "Pos", [ new Double ( "", $pos [0] - sin ( $getyaw / 180 * M_PI ) * $cos ),new Double ( "", $pos [1] + 1.6 ),new Double ( "", $pos [2] + cos ( $getyaw / 180 * M_PI ) * $cos * $f ) ] ),"Motion" => new Enum ( "Motion", [ new Double ( "", - sin ( $getyaw / 180 * M_PI ) * $cos * $f ),new Double ( "", - sin ( $getpitch / 180 * M_PI ) * $f ),new Double ( "", cos ( $getyaw / 180 * M_PI ) * $cos * $f ) ] ),"Rotation" => new Enum ( "Rotation", [ new Float ( "", $getyaw ),new Float ( "", $getpitch ) ] ) ] );
	...
	public function Skelleton_attack($i, $player, $f) {
		...
		if ($this->botspawnlist [$i] ["attackcool"] >= 6) {
			...
			$this->getServer ()->getScheduler ()->scheduleDelayedTask ( new CallbackTask ( [ $this,"removeArrow" ], [ $arrow ] ), 40 );
...
class Entities extends Creature {
	...
	public function spawnTo(Player $player) {
		...
		$this->setmotion_pk->entities = [ $this->getID (),$this->motionX,$this->motionY,$this->motionZ ];
