...
use codeinside\simpleelevator\Floor;
...
class SimpleElevator extends PluginBase implements Listener {
			...
			switch($mode) {
				...
				case self::MODE_ONGROUND:
					...
					if(count($blocks) > 0) {
						...
						$this->elevatorQueue[$e][self::TICK_BLOCKS] = [];
					}
	...
	private function advanceMove($player, $block, $event) {
		...
		if(mb_substr($text[0], 2) !== $this->msg("sign-output-title")) {
			$this->log("advanceMove-text this is not instance of Sign");
			return;
		}
		...
		if($target === false) {
			$this->log("advanceMove-target can't find target floor");
			return;
		}
	...
	public function createNewButton($floor) {
		...
		$this->log("createNewButton-type $type");
		$this->log("createNewButton-color $color");
		$this->log("createNewButton-next $next");
		$this->log("createNewButton-prev $prev");
		$this->log("createNewButton-index $index");
		$this->log("createNewButton-indexString $indexString");
	...
	public function parseIndex($string) {
		...
		$b = mb_substr($string, 0, 1);
		$absFloor = mb_substr($string, 1);
