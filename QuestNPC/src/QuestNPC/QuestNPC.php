...
class QuestNPC extends PluginBase implements Listener {
	...
	public function onEnable() {
		...
		$this->getServer ()->getScheduler ()->scheduleRepeatingTask ( new CallbackTask ( [ $this,"QuestNPC" ] ), 2 );
	...
	public function SignChange(SignChangeEvent $event) {
		...
		if (strtolower ( $event->getLine ( 0 ) ) != $this->get ( "QuestNPC-line0" )) return;
	...
	public function BlockBreak(BlockBreakEvent $event) {
		...
		if (! isset ( $this->db ["QuestNPC"] [$block->level->getFolderName ()] [$blockPos] )) return;
	...
	public function QuestNPC() {
		foreach ( $this->getServer ()->getOnlinePlayers () as $player ) {
			if (! isset ( $this->db ["QuestNPC"] [$player->level->getFolderName ()] ["nametag"] )) continue;
			foreach ( $this->db ["QuestNPC"] [$player->level->getFolderName ()] ["nametag"] as $tagPos => $message ) {
				...
				if (! ($dx <= 25 and $dy <= 25 and $dz <= 25)) {
					...
					if (isset ( $this->temp [$player->getName ()] ["nametag"] [$tagPos] )) {
						...
						$this->packet ["MovePlayerPacket"]->yaw = $getyaw;
						$this->packet ["MovePlayerPacket"]->pitch = $getpitch;
						$this->packet ["MovePlayerPacket"]->bodyYaw = $getyaw;
					...
					$this->packet ["AddPlayerPacket"]->yaw = $getyaw;
					$this->packet ["AddPlayerPacket"]->pitch = $getpitch;
