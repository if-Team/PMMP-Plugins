...
use ifteam\PSYCHOPASS\Event\CrimeCoefficient\getCrimeCoefficientEvent;
use ifteam\PSYCHOPASS\Event\CrimeCoefficient\setCrimeCoefficientEvent;
use ifteam\PSYCHOPASS\Event\CrimeCoefficient\clearCrimeCoefficientEvent;
use ifteam\PSYCHOPASS\Event\CrimeCoefficient\addCrimeCoefficientEvent;
use ifteam\PSYCHOPASS\Event\CrimeCoefficient\reduceCrimeCoefficientEvent;
...
class PSYCHOPASS_API extends PluginBase implements Listener {
	...
	public function onEnable() {
		if (! self::$instance instanceof PSYCHOPASS_API) self::$instance = $this;
	...
	public function onPlayerTouch(PlayerInteractEvent $event) {
		...
		if (! ($block->getID () == 323 or $block->getID () == 63 or $block->getID () == 68)) return;
		$sign = $event->getPlayer ()->getLevel ()->getTile ( $block );
		if (! ($sign instanceof Sign)) return;
		$sign = $sign->getText ();
		if (! ($sign [0] == "[색상체크]")) return;
		...
		if ((strtolower ( $command->getName () ) == "colorcheck") or ($command->getName () == "색상체크")) {
			if (isset ( $args [0] )) {
				if (isset ( $this->crime_con [$args [0]] )) {
					...
					if ($message == false) $message = TextFormat::DARK_AQUA . "[PSYCHOPASS-API] 해당되는 정보를 찾지 못했습니다.";
					...
					case "얻기" :
						if (isset ( $args [1] )) {
							...
							if ($target instanceof IPlayer) {
								...
								if (! $this->setCrimeCoefficient ( $args [1], 0, $sender )) $sender->sendMessage ( TextFormat::DARK_AQUA . "'" . $args [1] . "' 해당자를 찾지못했습니다." );
								return true;
							}
							return true;
						}
					...
					case "설정" :
						if (isset ( $args [1] ) and isset ( $args [2] ) and is_numeric ( $args [2] )) {
							...
							if ($target instanceof IPlayer) {
								...
								if (! $this->setCrimeCoefficient ( $args [1], $args [2], $sender )) $sender->sendMessage ( TextFormat::DARK_AQUA . "'" . $args [1] . "' 해당자를 찾지못했습니다." );
								return true;
							}
							return true;
						}
	...
	public function getColorMessage($player) {
		...
		switch ($checkcolor) {
			case "A" :
				if ($crime_coefficient < 25) $message .= TextFormat::WHITE . "흰색-색상 (CLEAR-COLOR)";
				else if ($crime_coefficient < 50) $message .= TextFormat::GRAY . "회색-색상 (GRAY-COLOR)";
				else if ($crime_coefficient < 100) $message .= TextFormat::DARK_GRAY . "짙은회색-색상 (DARKGRAY-COLOR)";
				else if ($crime_coefficient >= 100) $message .= TextFormat::BLACK . "흑색-색상 (BLACK-COLOR)";
				$color_descripts = "(흰색계열은 색상이 판별되지않는 유저일때 표시됩니다.)";
				break;
			case "B" :
				if ($crime_coefficient < 25) $message .= TextFormat::WHITE . "흰색-색상 (CLEAR-COLOR)";
				else if ($crime_coefficient < 50) $message .= TextFormat::RED . "적색-색상 (RED-COLOR)";
				else if ($crime_coefficient < 100) $message .= TextFormat::DARK_RED . "짙은적색-색상 (DARKRED-COLOR)";
				else if ($crime_coefficient >= 100) $message .= TextFormat::BLACK . "흑색-색상 (BLACK-COLOR)";
				$color_descripts = "(적색계열은 허용되지않는 공격이 많을 시 표시됩니다.)";
				break;
			case "C" :
				if ($crime_coefficient < 25) $message .= TextFormat::WHITE . "흰색-색상 (CLEAR-COLOR)";
				else if ($crime_coefficient < 50) $message .= TextFormat::YELLOW . "황색-색상 (YELLOW-COLOR)";
				else if ($crime_coefficient < 100) $message .= TextFormat::GOLD . "짙은황색-색상 (GOLD-COLOR)";
				else if ($crime_coefficient >= 100) $message .= TextFormat::BLACK . "흑색-색상 (BLACK-COLOR)";
				$color_descripts = "(황색계열은 허용되지않는 채팅(도배)이 많을 시 표시됩니다.)";
				break;
			case "D" :
				if ($crime_coefficient < 25) $message .= TextFormat::WHITE . "흰색-색상 (CLEAR-COLOR)";
				else if ($crime_coefficient < 50) $message .= TextFormat::GREEN . "녹색-색상 (GREEN-COLOR)";
				else if ($crime_coefficient < 100) $message .= TextFormat::DARK_GREEN . "짙은녹색-색상 (DARKGREEN-COLOR)";
				else if ($crime_coefficient >= 100) $message .= TextFormat::BLACK . "흑색-색상 (BLACK-COLOR)";
				$color_descripts = "(녹색계열은 허용되지않는 채팅(비속어)이 많을 시 표시됩니다.)";
				break;
			case "E" :
				if ($crime_coefficient < 25) $message .= TextFormat::WHITE . "흰색-색상 (CLEAR-COLOR)";
				else if ($crime_coefficient < 50) $message .= TextFormat::AQUA . "청록-색상 (AQUA-COLOR)";
				else if ($crime_coefficient < 100) $message .= TextFormat::DARK_AQUA . "짙은청록-색상 (DARKAQUA-COLOR)";
				else if ($crime_coefficient >= 100) $message .= TextFormat::BLACK . "흑색-색상 (BLACK-COLOR)";
				$color_descripts = "(청록계열은 허용되지않는 블럭설치가 많을 시 표시됩니다.)";
				break;
			case "F" :
				if ($crime_coefficient < 25) $message .= TextFormat::WHITE . "흰색-색상 (CLEAR-COLOR)";
				else if ($crime_coefficient < 50) $message .= TextFormat::BLUE . "남색-색상 (BLUE-COLOR)";
				else if ($crime_coefficient < 100) $message .= TextFormat::DARK_BLUE . "짙은남색-색상 (DARKBLUE-COLOR)";
				else if ($crime_coefficient >= 100) $message .= TextFormat::BLACK . "흑색-색상 (BLACK-COLOR)";
				$color_descripts = "(남색계열은 허용되지않는 블럭파괴가 많을 시 표시됩니다.)";
				break;
			case "G" :
				if ($crime_coefficient < 25) $message .= TextFormat::WHITE . "흰색-색상 (CLEAR-COLOR)";
				else if ($crime_coefficient < 50) $message .= TextFormat::LIGHT_PURPLE . "자색-색상 (LIGHTPURPLE-COLOR)";
				else if ($crime_coefficient < 100) $message .= TextFormat::DARK_PURPLE . "짙은자색-색상 (DARKPURPLE-COLOR)";
				else if ($crime_coefficient >= 100) $message .= TextFormat::BLACK . "흑색-색상 (BLACK-COLOR)";
	...
	public function addCrimeCoefficient($player, $crimecoefficient, $issuer = null) {
		...
		if (isset ( $this->crime_con [$name] )) {
			...
			if ($event->isCancelled ()) return false;
			$this->crime_con [$name] ["crime_coefficient"] += $crimecoefficient;
			if ($this->crime_con [$name] ["crime_coefficient"] < 0) $this->crime_con [$name] ["crime_coefficient"] = 0;
			return true;
		} else {
			return false;
		}
	...
	public function initialize_schedule_repeat($class, $method, $second, $param) {
		$this->getServer ()->getScheduler ()->scheduleRepeatingTask ( new CallbackTask ( [ $class,$method ], [ $param ] ), $second );
	}
